...
import ml.dmlc.xgboost4j.java.{XGBoostTMO => JXGBoostTMO, XGBoostError}
...
object XGBoostTMO {
  ...
  def train(
    ...
    val xgboostInJava = JXGBoostTMO.train(
      dtrain.jDMatrix,
  ...
  def loadModel(modelPath: String): Booster = {
    val xgboostInJava = JXGBoostTMO.loadModel(modelPath)
    new Booster(xgboostInJava)
  }
  ...
  def loadModel(in: InputStream): Booster = {
    val xgboostInJava = JXGBoostTMO.loadModel(in)
    new Booster(xgboostInJava)
  }
}
